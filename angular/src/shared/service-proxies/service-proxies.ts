...
@Injectable()
export class CalculationServiceServiceProxy {
    private http: Http;
    private baseUrl: string;
    protected jsonParseReviver: (key: string, value: any) => any = undefined;
     ...
     * @return Success
     */
    calculation_Add(x: number, y: number): Observable<number> {
        let url_ = this.baseUrl + "/api/services/app/CalculationService/Calculation_Add?";
        if (x !== undefined)
        ...
        return this.http.request(url_, options_).flatMap((response_) => {
            return this.processCalculation_Add(response_);
        }).catch((response_: any) => {
            if (response_ instanceof Response) {
                try {
                    return this.processCalculation_Add(response_);
    ...
    protected processCalculation_Add(response: Response): Observable<number> {
        const status = response.status; 
    ...
    plugin(): Observable<PluginObject[]> {
        ...
        return this.http.request(url_, options_).flatMap((response_) => {
            ...
            if (response_ instanceof Response) {
                ...
                return <Observable<PluginObject[]>><any>Observable.throw(response_);
        });
    }
        ...
        if (status === 200) {
            ...
            if (resultData200 && resultData200.constructor === Array) {
                ...
                for (let item of resultData200)
                    result200.push(PluginObject.fromJS(item));
            }
            return Observable.of(result200);
        } else if (status !== 200 && status !== 204) {
            const _responseText = response.text();
...
export class PluginObject implements IPluginObject {
    title: string;
    path: string;
    url: string;

    constructor(data?: IPluginObject) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (<any>this)[property] = (<any>data)[property];
    ...
    static fromJS(data: any): PluginObject {
        let result = new PluginObject();
        result.init(data);
        return result;
    }
